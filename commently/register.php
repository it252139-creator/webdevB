...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $password === '') {
        ...
        if ($error === '') {
            ...
            try {
                $dbh = db_open();
                $sql = 'INSERT INTO users (username, password, icon_path, color, created_at) VALUES (:u, :p, :i, :c, :created_at)';
                $stmt = $dbh->prepare($sql);
                $stmt->bindValue(':u', $username);
                $stmt->bindValue(':p', $hash);
                $stmt->bindValue(':i', $iconPath);
                $stmt->bindValue(':c', $color);
                $stmt->bindValue(':created_at', (new DateTimeImmutable('now', new DateTimeZone('Asia/Tokyo')))->format('Y-m-d H:i:s'));
                $stmt->execute();
